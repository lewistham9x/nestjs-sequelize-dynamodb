  ...
  class DynamoModel extends Model {
    ...
    static async findAll(q: Options | undefined) {
      ...
      do {
        ...
        await scan.exec().then((r: Result<any>) => {
          result.push.apply(
            result,
            r.map((x) => DynamoModel.jsonFy(new this(x))),
          );
          result.queryCount++;
    ...
    static async find(query: Options, _limit: Number) {
      ...
      const { op = 'eq' } = query;
      const q = Object.keys(query.where).reduce((prev: Options, k: string) => {
        prev[k] = {
          [op]: query.where[k],
        };
        return prev;
    ...
    static async update(update: Options, query: Options) {
      return await new Promise((resolve, reject) => {
        DynamoModel.Model.update(
          query.where,
          DynamoModel.parser(update),

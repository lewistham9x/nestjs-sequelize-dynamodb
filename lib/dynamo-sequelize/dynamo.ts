...
export function seqSchemaToDynamoSchema(
  seqSchema: Options,
...
export class Dynamo extends Sequelize {
  options: Options;
  ...
  define(name: string, seqSchema: Options) {
    ...
    const { config, jsonTypes } = seqSchemaToDynamoSchema(
      seqSchema,
      jsonAsObject,
    );
    ...
    const model = dynamoose.model(name, sc);
